 ...
 */
function toWhatsAppTwilioAddress(raw) {
...
function shouldEmitOtpToConsoleOnly() {
    return (process.env.NODE_ENV === "development" && process.env.AUTH_OTP_CONSOLE === "1");
}
export async function sendOtpMessage(params) {
    ...
    try {
        ...
        const twilioCode = err && typeof err === "object" && "code" in err ? err.code : undefined;
        const twilioStatus = err && typeof err === "object" && "status" in err ? err.status : undefined;
        logger.error({
