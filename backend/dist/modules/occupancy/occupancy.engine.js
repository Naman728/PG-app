...
export function maintenanceActiveForBed(b) {
    if (b.status === BedStatus.UNDER_MAINTENANCE)
        return true;
    if (b.maintenanceNote?.trim())
        return true;
    return b.maintenanceTickets.some((t) => ACTIVE_MAINTENANCE.includes(t.status));
}
export function computeRentDisplayStatus(b, now) {
    if (b.status === BedStatus.VACANT || !b.tenantUserId)
        return "NONE";
    const sod = startOfLocalDay(now).getTime();
    const actionable = b.rentInvoices.filter((inv) => inv.status === RentInvoiceStatus.DUE || inv.status === RentInvoiceStatus.OVERDUE);
    for (const inv of actionable) {
        if (inv.status === RentInvoiceStatus.OVERDUE)
            return "OVERDUE";
        const due = inv.dueDate.getTime();
        if (inv.status === RentInvoiceStatus.DUE && due < sod)
            return "OVERDUE";
    }
    ...
    if (b.status === BedStatus.OCCUPIED_UNPAID) {
        if (!b.paidThrough)
            return "OVERDUE";
        if (b.paidThrough.getTime() < sod)
            return "OVERDUE";
        return "PENDING";
    }
    return "CURRENT";
}
export function computeBedOperationalTone(b, now) {
    if (maintenanceActiveForBed(b))
        return "MAINTENANCE";
    if (b.status === BedStatus.VACANT || !b.tenantUserId)
        return "VACANT";
...
export function computeRoomOccupancyStatus(beds, now) {
    if (!beds.length)
        return "VACANT";
    const tones = beds.map((b) => computeBedOperationalTone(b, now));
    if (tones.some((t) => t === "MAINTENANCE"))
        return "MAINTENANCE";
    const vac = tones.filter((t) => t === "VACANT").length;
    if (vac === beds.length)
        return "VACANT";
    if (vac > 0)
        return "PARTIAL_VACANCY";
    if (tones.some((t) => t === "OVERDUE"))
        return "OVERDUE";
    if (tones.some((t) => t === "PENDING"))
        return "PENDING";
    return "OCCUPIED_PAID";
}
/** Maps derived room status to legacy Prisma aggregate for filters / backwards compatibility. */
export function occupancyStatusToLegacyAggregate(s) {
    switch (s) {
        case "MAINTENANCE":
...
//# sourceMappingURL=occupancy.engine.js.map