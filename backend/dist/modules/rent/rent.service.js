...
export function computeDueDate(billingYear, billingMonth, dueDayOfMonth) {
...
export async function ensureRentReminderSettings(organizationId) {
...
export async function generateMonthlyRentInvoicesForOrg(params) {
    ...
    const dueDate = computeDueDate(params.billingYear, params.billingMonth, settings.dueDayOfMonth);
...
export async function markOverdueInvoicesGlobally() {
...
async function hasRecentQueuedTenantReminder(params) {
    ...
    return recent.some((j) => j.payload?.invoiceId === params.invoiceId);
}
...
export async function runRentReminderSweepForOrg(organizationId) {
    ...
    for (const inv of overdueInvoices) {
        ...
        const ok = !last || daysBetweenUtc(last, today) >= settings.overdueRepeatDays;
        if (ok) {
            await enqueueTenantRentReminder({
                organizationId,
                invoiceId: inv.id,
...
export async function runRentReminderSweepAllOrgs() {
...
export async function generateCurrentMonthForAllOrgs() {
    const now = new Date();
...
export async function getRentDashboard(params) {
    ...
    const collectionRate = expectedMinor > 0 ? Math.min(1, collectedMinor / expectedMinor) : paid > 0 ? 1 : 0;
    return {
        billingYear,
        billingMonth,
...
export async function listRentInvoices(params) {
    ...
    if (query.billingYear != null)
        where.billingYear = query.billingYear;
    if (query.billingMonth != null)
...
export async function listTenantRentInvoices(userId) {
