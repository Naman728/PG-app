import { MaintenanceMessageVisibility, MaintenanceTicketStatus, OrgRole, } from "@prisma/client";
import { badRequest, forbidden, notFound } from "../../common/httpErrors.js";
import { prisma } from "../../prisma/client.js";
import { signedAuthenticatedUrl, uploadMaintenanceTicketPhoto, } from "../../services/cloudinary.service.js";
import { MAINTENANCE_ACTIVITY } from "./maintenance.constants.js";
import { notifyAssignee, notifyManagersNewTicket, notifyTenantTicketResolved, } from "./maintenance-notification.helper.js";
const STATUS_FLOW = {
    OPEN: [
...
export async function assertTenantInOrg(organizationId, userId) {
    const m = await prisma.organizationMember.findFirst({
        where: { organizationId, userId, orgRole: OrgRole.TENANT },
    });
    if (!m)
        throw forbidden("Not a tenant of this organization");
}
export async function assertStaffInOrg(organizationId, userId) {
    const m = await prisma.organizationMember.findFirst({
        where: {
            organizationId,
            userId,
    ...
    if (!m)
        throw forbidden("No staff access for this organization");
}
async function assertBedInOrg(organizationId, bedId) {
    ...
    if (!bed)
        throw badRequest("Bed not found in this organization");
}
...
export async function listMaintenanceTicketsForOrg(params) {
    ...
    if (params.query.priority)
        where.priority = params.query.priority;
    if (params.query.assignedToUserId)
        where.assignedToUserId = params.query.assignedToUserId;
...
export async function getMaintenanceTicketForOrg(organizationId, ticketId) {
...
export async function getMaintenanceTicketForTenant(organizationId, ticketId, tenantUserId) {
...
export async function getMaintenanceTimeline(params) {
    ...
    const merged = [
        ...activities.map((a) => ({ kind: "activity", createdAt: a.createdAt, data: a })),
        ...messages.map((m) => ({ kind: "message", createdAt: m.createdAt, data: m })),
    ];
...
export async function patchMaintenanceTicketStatus(params) {
    ...
    const reopening = params.status === MaintenanceTicketStatus.IN_PROGRESS &&
...
export async function addMaintenanceMessage(params) {
    ...
    if (!params.authorIsStaff) {
        if (ticket.tenantUserId !== params.authorUserId)
            throw forbidden("Cannot comment on this ticket");
        visibility = MaintenanceMessageVisibility.TENANT;
    }
    else {
...
export async function listMaintenanceStaffAssignees(organizationId) {
...
export async function getMaintenanceMetrics(params) {
    ...
    return {
        ...
        avgResolutionHours: resolutionCount > 0 ? totalResolutionMs / resolutionCount / 3_600_000 : null,
