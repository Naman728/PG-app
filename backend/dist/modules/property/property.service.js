...
function bedLabelsForSharing(sharing) {
...
export class PropertyService {
    repo;
    constructor(repo = new PropertyRepository()) {
        this.repo = repo;
    }
    /** Delegates to floor-map module (single operational source of truth). */
    async getMap(organizationId) {
     ...
     */
    async bulkSetup(organizationId, input) {
        ...
        if (existingBeds > 0) {
            throw badRequest("Your property already has beds on the map. Add more rooms from the floor map, or delete existing rooms/beds first if you want to run quick setup again.");
        }
        const planned = [];
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const existingNames = new Set((await tx.room.findMany({
    ...
    async getStats(organizationId) {
        ...
        const occupancyRate = totalBeds === 0 ? 0 : (totalBeds - vacantBeds) / totalBeds;
        return {
            counts,
            totalBeds,
            occupancyRate,
    ...
    async createFloor(organizationId, input) {
        ...
        if (!name)
            throw badRequest("Floor name is required");
    ...
    async updateFloor(organizationId, floorId, input) {
        const floor = await this.repo.assertFloorInOrg(floorId, organizationId);
        if (!floor)
            throw notFound("Floor not found");
        return this.repo.updateFloor(floorId, input);
    }
    async deleteFloor(organizationId, floorId) {
        const floor = await this.repo.assertFloorInOrg(floorId, organizationId);
        if (!floor)
            throw notFound("Floor not found");
        await prisma.$transaction(async (tx) => {
    ...
    async createRoom(organizationId, floorId, input) {
        const floor = await this.repo.assertFloorInOrg(floorId, organizationId);
        if (!floor)
            throw notFound("Floor not found");
    ...
    async updateRoom(organizationId, roomId, input) {
        ...
        if (input.sharingLabel !== undefined)
            data.sharingLabel = input.sharingLabel;
        if (input.colStart !== undefined)
            data.colStart = input.colStart;
        if (input.colSpan !== undefined)
            data.colSpan = input.colSpan;
        if (input.rowStart !== undefined)
            data.rowStart = input.rowStart;
        if (input.rowSpan !== undefined)
            data.rowSpan = input.rowSpan;
    ...
    async deleteRoom(organizationId, roomId) {
        const room = await this.repo.assertRoomInOrg(roomId, organizationId);
        if (!room)
            throw notFound("Room not found");
        await prisma.$transaction(async (tx) => {
    ...
    async createBed(organizationId, roomId, input) {
        const room = await this.repo.assertRoomInOrg(roomId, organizationId);
        if (!room)
            throw notFound("Room not found");
    ...
    async updateBed(organizationId, bedId, input) {
        const bed = await this.repo.assertBedInOrg(bedId, organizationId);
        if (!bed)
            throw notFound("Bed not found");
        const data = {};
    ...
    async deleteBed(organizationId, bedId) {
        const bed = await this.repo.assertBedInOrg(bedId, organizationId);
        if (!bed)
            throw notFound("Bed not found");
        return this.repo.softDeleteBed(bedId);
    }
    async assignBed(organizationId, bedId, input) {
        const bed = await this.repo.assertBedInOrg(bedId, organizationId);
        if (!bed)
            throw notFound("Bed not found");
        ...
        const member = await this.repo.assertTenantMember(organizationId, input.tenantUserId);
        if (!member) {
    ...
    async vacateBed(organizationId, bedId) {
        const bed = await this.repo.assertBedInOrg(bedId, organizationId);
        if (!bed)
            throw notFound("Bed not found");
    ...
    async markBedPaid(organizationId, bedId, input) {
        const bed = await this.repo.assertBedInOrg(bedId, organizationId);
        if (!bed)
            throw notFound("Bed not found");
        if (!bed.tenantUserId) {
