...
}): Promise<{ publicId: string; resourceType: "image" | "raw"; bytes: number }> {
  ensureConfigured();
...
export function signedAuthenticatedUrl(
  publicId: string,
...
export async function uploadMaintenanceTicketPhoto(params: {
  ...
  mimeType: string;
  originalFilename: string;
}): Promise<{ publicId: string; resourceType: "image"; bytes: number }> {
...
export async function destroyAuthenticatedDocument(
  publicId: string,
  resourceType: "image" | "raw",
): Promise<void> {
  ensureConfigured();
