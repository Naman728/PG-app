...
} from "../maintenance/maintenance.constants.js";
import {
...
async function tryWhatsAppThenSms(params: {
  ...
  if (!params.smsFallbackEnabled) return false;
...
async function processTenantReminder(job: NotificationJob): Promise<void> {
  const payload = job.payload as RentTenantReminderPayload;
  ...
  const settings =
    (await prisma.rentReminderSettings.findUnique({
...
async function processOwnerDigest(job: NotificationJob): Promise<void> {
  const payload = job.payload as RentOwnerDigestPayload;
...
async function loadOrgNotificationPrefs(organizationId: string) {
...
async function processMaintenanceStaffAlert(job: NotificationJob): Promise<void> {
  const payload = job.payload as MaintenanceStaffAlertPayload;
  const user = await prisma.user.findFirst({
    where: { id: payload.recipientUserId, deletedAt: null },
...
async function processMaintenanceTenantAlert(job: NotificationJob): Promise<void> {
  const payload = job.payload as MaintenanceTenantAlertPayload;
  const user = await prisma.user.findFirst({
    where: { id: payload.recipientUserId, deletedAt: null },
    select: { id: true, phone: true },
  });
