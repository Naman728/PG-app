import type { Prisma, RentInvoiceStatus } from "@prisma/client";
...
import {
  bulkEnqueueReminders,
  confirmRentPayment,
  ensureRentReminderSettings,
  generateMonthlyRentInvoicesForOrg,
  getRentDashboard,
  getRentInvoiceForReceipt,
  listNotificationJobsForOrg,
  listRentInvoices,
} from "./rent.service.js";

export const getRentReminderSettings = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
  const settings = await ensureRentReminderSettings(orgId);
  sendSuccess(res, { settings });
});

export const patchRentReminderSettings = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
  const body = req.body as UpdateRentReminderSettingsInput;
...
export const getRentDashboardHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
...
export const listRentInvoicesHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
...
export const confirmRentPaymentHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
  const invoiceId = req.params.invoiceId;
  const body = req.body as ConfirmRentPaymentInput;
...
export const bulkRemindRentHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
...
export const generateRentMonthHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
  const now = new Date();
...
export const exportRentExcelHandler = asyncHandler(async (req: Request, res: Response) => {
  const orgId = req.params.orgId;
  const status = req.query.status as RentInvoiceStatus | undefined;
  ...
  if (billingYear != null && !Number.isNaN(billingYear)) where.billingYear = billingYear;
  if (billingMonth != null && !Number.isNaN(billingMonth)) where.billingMonth = billingMonth;

  const buf = await buildRentInvoicesWorkbook({ organizationId: orgId, where });
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="rent-invoices-${orgId.slice(0, 8)}.xlsx"`,
  );
  res.setHeader(
    "Content-Type",
...
export const downloadRentReceiptPdfHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="receipt-${inv.receiptNumber ?? invoiceId}.pdf"`,
    );
    res.send(pdf);
  },
);

export const listRentNotificationJobsHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const orgId = req.params.orgId;
