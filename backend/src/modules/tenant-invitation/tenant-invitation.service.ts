...
import {
  createInviteTokenValue,
  generateNumericOtp,
  hashOtp,
  hashRefreshToken,
  verifyOtp,
} from "../auth/cryptoTokens.js";
...
export class TenantInvitationService {
  constructor(
    private readonly invites = new TenantInvitationRepository(),
    private readonly authRepo = new AuthRepository(),
    private readonly authService = new AuthService(),
  ) {}

  private resolveTokenHash(rawToken: string) {
  ...
  async requestInviteOtp(rawToken: string) {
    const tokenHash = this.resolveTokenHash(rawToken);
    const invite = await this.invites.findActiveByTokenHash(tokenHash);
    if (!invite) {
      throw notFound("Invitation not found or expired");
  ...
  async verifyInviteOtp(
    rawToken: string,
    input: InvitationVerifyOtpInput,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (user) {
        if (user.role === UserRole.OWNER || user.role === UserRole.STAFF) {
          throw forbidden(
            "This phone is registered as owner/staff. Use owner login.",
          );
        }
  ...
  async createInvite(params: {
    ...
    const { organizationId, invitedByUserId, input, inviterDisplayName } =
      params;
