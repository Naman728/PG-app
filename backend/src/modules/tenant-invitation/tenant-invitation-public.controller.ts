import type { InvitationVerifyOtpInput } from "@pg-manager/shared";
...
export const verifyInvitationOtp = asyncHandler(async (req, res) => {
  const body = req.body as InvitationVerifyOtpInput;
  const { user, accessToken, refreshToken } =
    await tenantInvites.verifyInviteOtp(req.params.token, body, {
