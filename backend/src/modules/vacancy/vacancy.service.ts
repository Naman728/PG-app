...
function matchesVacancyRoomFilter(
  room: {
    occupancyStatus: string;
    vacantBeds: number;
    beds: Array<{ maintenanceActive: boolean }>;
  },
  filter: VacancyRoomFilter,
): boolean {
  if (room.vacantBeds <= 0) return false;
...
export class VacancyService {
  async summary(organizationId: string) {
    try {
      ...
      let oldestVacantMs: number | null = null;
  ...
  async rooms(organizationId: string, filter: VacancyRoomFilter) {
    try {
      const { floors, meta } = await floorMapService.getOperationalFloors(organizationId);
      const rooms: Array<{
        floor: { id: string; name: string };
        room: (typeof floors)[number]["rooms"][number];
      }> = [];
      for (const f of floors) {
        for (const r of f.rooms) {
          if (!matchesVacancyRoomFilter(r, filter)) continue;
